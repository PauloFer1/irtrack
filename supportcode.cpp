	...
	LPARAM	lParam)			// Additional Message Information
{
	...
	case WM_POWERBROADCAST:
		// REPORT(Debug::General,">>>>>>> WM_POWERBROADCAST <<<<<<<");
		if(wParam == PBT_APMSUSPEND)
		{
		//REPORT(Debug::General,"--> System is suspending <--");
		CameraLibrary::CameraManager::X().PrepareForSuspend();
		}
		if(wParam == PBT_APMRESUMEAUTOMATIC)
		{
		//REPORT(Debug::General,"--> System is resuming <--");
...
BOOL CreateAppWindow(const char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	if (!(hWnd = CreateWindowEx(dwExStyle,							// Extended Style For The Window
		L"OpenGL",							// Class Name
		L"IR Camera Tracking",								// Window Title
...
bool FullscreenToggle()
{
	...
	{
		MessageBox(0, L"Unable to toggle to full screen", L"Error", MB_OK);
		return 1;
	}
